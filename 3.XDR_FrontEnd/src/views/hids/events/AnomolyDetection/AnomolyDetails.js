...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-event-anomaly-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&agent_id=${searchParams.get('agent_id')}&agent_ip=${searchParams.get('agent_ip')}&anomaly_label_count=${searchParams.get('anomaly_label_count')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
