...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-event-security-event-page-top-rule-groups-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&agent_id=${searchParams.get('agent_id')}&rule_groups=${searchParams.get('rule_groups')}&agent_ip=${searchParams.get('agent_ip')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
