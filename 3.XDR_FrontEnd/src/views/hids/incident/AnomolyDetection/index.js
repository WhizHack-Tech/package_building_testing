...
const Nidstype = () => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-hids-anomaly-label?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='xl'>
          <ModalHeader>{t('Anomaly ')}</ModalHeader>
          <ModalBody>
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <span className='align-middle fw-bold'>Anomoly Detection</span>
                        </td>
                        <td>
                          <span className='align-middle fw-bold'>{rows.agent_ip}</span>
                        </td>
                        <td>
                          <span className='align-middle fw-bold'>{rows.anomaly_label_count}</span>
                        </td>
                        <td>
                          <Link to={`/hids-incident-anomaly-details?current_time=${rows.current_time}&past_time=${rows.past_time}&agent_id=${rows.agent_id}&agent_ip=${rows.agent_ip}&anomaly_label_count=${rows.anomaly_label_count}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
