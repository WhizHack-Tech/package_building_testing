// ================================================================================================
//  File Name: index.js
//  Description: Details of the HIDS Incident Graph (Mite PIC DSS).
//  ----------------------------------------------------------------------------------------------
...
const ApexRadiarChart = () => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-hids-rule-pci-dss-pie?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='xl'>
          <ModalHeader>{t('PCI DSS')}</ModalHeader>
          <ModalBody>
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <span className='align-middle fw-bold'>{rows.rule_pci_dss}</span>
                        </td>
                        <td>
                          <span className='align-middle fw-bold'>{rows.rule_pci_dss_count}</span>
                        </td>
                        <td>
                          <Link to={`/hids-incident-pic-dss-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.agent_id}&name1=${rows.rule_pci_dss}&name1=${rows.rule_pci_dss}&name2=${rows.agent_ip}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
