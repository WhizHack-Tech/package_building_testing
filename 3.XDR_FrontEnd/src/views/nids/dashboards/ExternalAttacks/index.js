...
import Critical from "../../../../assets/images/svg/external.svg"
...
const CriticalThreats = () => {
    ...
    const [externalThreatsTotal, setExternalThreatsTotal] = useState(0)
    ...
    const apiLogic = () => {
        ...
        axios.get(`/nids-dashboard-ext?condition=${filterState.values ? filterState.values : 'today'}`)
            .then(res => {
                setApiLoader(false)
                if (res.data.message_type === "success") {
                    setExternalThreatsTotal(res.data.attack_count)
    ...
    return (
        <Card>
            ...
            <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
                <ModalHeader>External Attacks</ModalHeader>
                <ModalBody>
                                ...
                                shortTableData.map((rows, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <Link to={`/external-attacks-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.threat_name}`}>
            ...
            <CardBody>
                ...
                <Avatar color='light-warning' icon={<img width={40} src={Critical} />} size='lg'/>
                    <h1>{externalThreatsTotal}</h1>
                    <p>Attack Events</p>
                </div>
            </CardBody>
