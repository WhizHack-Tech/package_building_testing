...
const DetectedThreatType = () => {
    ...
    const apiLogic = () => {
        ...
        axios.get(`/nids-alert-detected-threat-card?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
                        ...
                        tableData.map((rows, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>
                                        <Link to={`/nids-alerts-ids-detected-threat-details?current_time=${rows.current_time}&past_time=${rows.past_time}&type=${type}&name=${rows.name}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                                            More Details
                                        </Link>
                                    </td>
                                </tr>
