...
const GoogleMaps = () => {
    ...
    const apiLogic = () => {
        ...
        axios.get(`/nids-alert-geo?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
                                ...
                                shortTableData.map((rows, index) => {
                                    ...
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <Link to={`/nids-alert-ids-geo-location-details?current_time=${rows.current_time}&past_time=${rows.past_time}&geoip_city=${rows.geoip_city}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                                                    {t('More Details')}
                                                </Link>
                                            </td>
                                        </tr>
