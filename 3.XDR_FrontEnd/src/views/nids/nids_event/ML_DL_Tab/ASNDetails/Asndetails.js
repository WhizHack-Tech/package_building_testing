...
//  Description: Details of the NIDS Events ML & DL ( ASN Details )
//  ----------------------------------------------------------------------------------------------
...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/asn-table-nids-event-ml?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&geoip_asn_name=${searchParams.get('geoip_asn_name')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
