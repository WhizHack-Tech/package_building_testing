...
const ApexRadiarChart = () => {
  ...
  const apiLogic = () => {
    setApiLoader(true)
    axios.get(`/nids-incident-target-port-pie?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-incident-target-port-details?current_time=${rows.current_time}&past_time=${rows.past_time}&target_port=${rows.target_port}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
