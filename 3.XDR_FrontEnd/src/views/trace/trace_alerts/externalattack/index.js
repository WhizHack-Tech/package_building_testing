...
const ExternalAttackchart = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/trace-alert-ext?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                    ...
                    filteredData.map((rows, index) => {
                      return (
                        ...
                        <td>
                          ...
                          <td>
                            <Link to={`/trace-alert-external-attack-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                              More Details
                            </Link>
                          </td>
                        </tr>
