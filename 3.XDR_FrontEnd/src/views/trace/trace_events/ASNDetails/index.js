// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Events ( ASN Attacker Details )).
//  ----------------------------------------------------------------------------------------------
...
const ANSDetailschart = ({ tooltipShadow, gridLineColor, labelColor, successColorShade, lineChartPrimary, yellowColor }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/attker-asn-trace-event?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
  ...
  rows.geoip_asn_name.toLowerCase().includes(searchQuery.toLowerCase())
)
    ...
    <Card>
      <CardHeader className='d-flex justify-content-between align-items-sm-center align-items-start flex-sm-row flex-column'>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='xl'>
          <ModalHeader>{t('Top Source Attacker ASN ')}</ModalHeader>
          ...
          <ModalBody>
            <Table striped responsive>
              <thead>
                <tr>
                  <th>{t('Sensor Name')}</th>
                  <th>{t('Top Source Attacker ASN ')}</th>
                  <th>{t('Counts')}</th>
                  <th>{t('Actions')}</th>
                </tr>
              </thead>
                    ...
                    filteredData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/trace-events-ans-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.geoip_asn_name}&name1=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                              {t('More Details')}
                            </Link>
                          </td>
                        </tr>
...
export default ANSDetailschart
