// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Events ( Critical Threats )).
//  ----------------------------------------------------------------------------------------------
...
const criticalThreats = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/critical-threats-trace-event?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  (shortTableData.length === 0 || 
                    ...
                    filteredData.map((rows, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/trace-events-critical-threats-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.type_of_threat}&name1=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
