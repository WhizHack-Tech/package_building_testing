// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Incidents ( Lateral Movement Details )).
//  ----------------------------------------------------------------------------------------------
...
const LateralMovementchart = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-lateral-mov-trace?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader className='align-items-start pb-0'>
        <div>
          ...
          <CardText className='mb-1'>{t('Lateral Movement')}</CardText>
        </div>
                    ...
                    filteredData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/trace-incident-lateral-movement-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                              {t('More Details')}
                            </Link>
                          </td>
                        </tr>
