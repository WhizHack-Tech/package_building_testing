...
import { filterValues } from '../../../views/hids/store/incident_charts'
let setIntervalFun = null
const BreadCrumbs = props => {
  const { t } = useTranslation()
  const { breadCrumbTitle } = props
  const [isOpen, setIsOpen] = useState(false)
  const [filters, setFilters] = useState({ name: t('Last 1 hour'), value: 'last_1_hour' })
  const [reloadVal, setReloadVal] = useState('off')
  const [timeVal, setTimeVal] = useState(1)
  const filterStore = useSelector(state => state.incident_charts)
