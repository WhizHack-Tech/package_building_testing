...
//  Description: Details of the breadcrumbs merge component.
// ----------------------------------------------------------------------------------------------
...
const BreadCrumbs = props => {
  ...
  const filter_val_show = useSelector((store) => store.dashboard_charts.filterValue)
  const [startDatePicker, setStartDatePicker] = useState(0)
  const [endDatePicker, setEndDatePicker] = useState(0)
  const [basicModal, setBasicModal] = useState(false)
  const loadding = useSelector((store) => store.dashboard_charts.loadding)
  ...
  const filter_form = () => {
    ...
    if (defaultFilter === true) {
      ...
      if (checkPlateformVal === false) {
        ...
        dispatch(dashboar_charts({ platform: filter_val_show.defaultVals.platformSetVal, threat_severity: threatSeverity, start_date, end_date }))
        dispatch(dashboar_filter({ ...filter_value, platform: filter_val_show.platform }))
      }

      if (checkSeverityVal === false) {
        threatSeverityHandle(filter_val_show.defaultVals.severityVal)
        dispatch(dashboar_charts({ platform, threat_severity: filter_val_show.defaultVals.severitySetVal, start_date, end_date }))
        dispatch(dashboar_filter({ ...filter_value, threat_severity: filter_val_show.threat_severity }))
      }

      if (checkPlateformVal && checkSeverityVal) {
        dispatch(dashboar_charts({ platform, threat_severity: threatSeverity, start_date, end_date }))
        dispatch(dashboar_filter(filter_value))
      }

    } else {
      dispatch(dashboar_charts({ platform: filter_val_show.defaultVals.platformSetVal, threat_severity: filter_val_show.defaultVals.severitySetVal, start_date, end_date}))
      dispatch(dashboar_filter({...filter_val_show, start_date, end_date}))
    }
  ...
  return (
    <div className='content-header row inline'>
      <div className='content-header-left col-md-2 col-12 mb-1'>
        <div className='row breadcrumbs-top'>
          <div className='col-12'>
            {breadCrumbTitle ? <h3>{breadCrumbTitle}</h3> : ''}
          </div>
        </div>
      </div>
