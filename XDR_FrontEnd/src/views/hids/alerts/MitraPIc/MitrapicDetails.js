...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-alert-rule-pci-dss-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}&name1=${searchParams.get('name1')}&name2=${searchParams.get('name2')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
