...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-alert-mitre-attack-event-page-rule-mitre-technique-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&agent_id=${searchParams.get('agent_id')}&rule_mitre_technique=${searchParams.get('rule_mitre_technique')}&agent_ip=${searchParams.get('agent_ip')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
