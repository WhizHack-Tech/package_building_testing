...
const TablesData = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-event-agent-id-details?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    const basicColumns = [
        ...
        },
        {
            ...
            selector: 'action',
            cell: rowData => {
                return <Link to={`/hids-event-details?current_time=${rowData.current_time}&past_time=${rowData.past_time}&agent_id=${rowData.agent_id}&condition=${rowData.condition}`}>
                    <Eye />
                </Link>
            }
        }
    ...
    return (
        <Fragment>
            <Card>
                <CardHeader>
                    <CardTitle tag='h2'>{t('Events')}</CardTitle>
                </CardHeader>
