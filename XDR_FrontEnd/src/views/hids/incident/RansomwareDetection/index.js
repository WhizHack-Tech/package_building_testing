...
const Nidstype = () => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-hids-potential-ransomware?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/hids-incident-ransomware-details?current_time=${rows.current_time}&past_time=${rows.past_time}&agent_id=${rows.agent_id}&agent_ip=${rows.agent_ip}&ransomware_count=${rows.ransomware_count}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
