...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/hids-incident-mitre-evolution-time-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&agent_id=${searchParams.get('agent_id')}&rule_mitre_tactic=${searchParams.get('rule_mitre_tactic')}&agent_ip=${searchParams.get('agent_ip')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
             <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                <CardTitle tag='h4'> {t("Mitre Alerts Evolution Over Time")} : {searchParams.get("agent_ip")}</CardTitle>
