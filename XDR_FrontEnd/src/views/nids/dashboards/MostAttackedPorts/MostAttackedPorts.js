...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-attck-event-port-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`)
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
            <CardHeader>
                <CardTitle>
                Most Attacked Ports : {searchParams.get('name')}
                </CardTitle>
            </CardHeader>
