...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`nids-dashboard-outgoing-botnet-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`)
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
            <CardHeader>
                <CardTitle>
                OutgoingBotnet : {searchParams.get('name')}
                </CardTitle>
            </CardHeader>
