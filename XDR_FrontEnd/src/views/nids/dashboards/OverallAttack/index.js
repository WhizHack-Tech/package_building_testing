...
const PieCharts = () => {
  ...
  const ChartApiLogic = () => {
    ...
    axios.get(`/nids-dashboard-overall-attack-pie?condition=${filterState.values ? filterState.values : 'today'}`)
      .then(res => {
        setApiLoader(false)
        if (res.data.message_type === "success") {
          ...
          const chartOption = {
            ...
            labels: res.data.piechart.labels,
          ...
          if (res.data.piechart.series.length > 0) {
            const tableDataRows = res.data.piechart.series.map((series, index) => ({
              series,
              labels: res.data.piechart.labels[index],
          ...
          setChartData({
            options: chartOption,
            series: res.data.piechart.series
          })
                ...
                shortTableData.map((rows, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        <Link to={`/overall-attack-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.labels}`}>
