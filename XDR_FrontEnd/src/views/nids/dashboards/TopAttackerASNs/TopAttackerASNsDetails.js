...
const TopAttackedASNDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/attacker-country-pie-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`)
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
            <CardHeader>
                <CardTitle>
                Overall Attack Comparison : {searchParams.get('name')}
                </CardTitle>
            </CardHeader>
...
export default TopAttackedASNDetails