// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Alerts ( Attack Count Details ).
//  ----------------------------------------------------------------------------------------------
...
const AttackCountchart = ({ direction, warning }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/attack-frequency-card-nids-alert?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-attack-frequency-details/?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.name}`} className='btn-sm btn-outline-primary m-2'>
                          {t('More Details')}
                          </Link>
                        </td>
                      </tr>
...
export default AttackCountchart
