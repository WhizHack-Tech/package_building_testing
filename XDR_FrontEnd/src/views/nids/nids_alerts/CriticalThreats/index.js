// ================================================================================================
//  File Name: CriticalThreats.js
//  Description: Details of the NIDS Alerts ( Critical Threats ).
//  ----------------------------------------------------------------------------------------------
...
const criticalThreats = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/critical-threats-count-nids-alert?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  (shortTableData.length === 0 || checkApiData === false) ? (
                    ...
                    shortTableData.map((rows, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-alert-critical-threats-details?current_time=${rows.current_time}&past_time=${rows.past_time}&type_of_threat=${rows.type_of_threat}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
