...
const TargetMacDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-alert-target-mac-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&target_mac=${searchParams.get('target_mac')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
