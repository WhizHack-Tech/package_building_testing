// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Alerts ( Service Names )
//  ----------------------------------------------------------------------------------------------
...
const ServiceAttacker = ({ tooltipShadow, gridLineColor, labelColor, successColorShade, lineChartPrimary, yellowColor }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-alert-service-name-bar-chart?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader className='d-flex justify-content-between align-items-sm-center align-items-start flex-sm-row flex-column'>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
          <ModalHeader>{t('Service Names')}</ModalHeader>
          <ModalBody>
            <Table striped responsive>
              <thead>
                <tr>
                  <th>{t('Service Names')}</th>
                  <th>{t('Platform')}</th>
                  <th>{t('Counts')}</th>
                  <th>{t('Actions')}</th>
                </tr>
                  ...
                  (shortTableData.length === 0 || checkApiData === false) ? (
                    ...
                    shortTableData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          <td>
                            <span className='align-middle fw-bold'>{rows.service_name}</span>
                          </td>
                          <td>
                            <span className='align-middle fw-bold'>{rows.platform}</span>
                          </td>
                          <td>
                            <span className='align-middle fw-bold'>{rows.service_name_count}</span>
                          </td>
                          <td>
                            <Link to={`/nids-alert-attacked-Services-details?current_time=${rows.current_time}&past_time=${rows.past_time}&service_name=${rows.service_name}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                            </Link>
                          </td>
                        </tr>
...
export default ServiceAttacker
