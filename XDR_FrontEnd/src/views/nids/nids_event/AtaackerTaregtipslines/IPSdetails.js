...
const AttackerIpsDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-event-attacker-ip-line-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&attacker_ip=${searchParams.get('attacker_ip')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
             <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                <CardTitle tag='h4'> {t("Attacker IPs")} : {searchParams.get("attacker_ip")}</CardTitle>
