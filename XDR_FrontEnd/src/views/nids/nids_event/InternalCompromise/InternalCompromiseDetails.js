...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-alert-int-compr-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
            <CardHeader>
                <CardTitle>
                    {t('Internal Attacks')} : {searchParams.get('name')}
                </CardTitle>
            </CardHeader>
