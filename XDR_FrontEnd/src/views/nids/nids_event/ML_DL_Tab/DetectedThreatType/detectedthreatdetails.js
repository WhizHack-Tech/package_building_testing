...
//  Description: Details of the NIDS Events ML & DL ( Detected Threat )
//  ----------------------------------------------------------------------------------------------
...
const DetectedThreatDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/detected-threat-table-nids-event-ml?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&type=${searchParams.get('type')}&name=${searchParams.get('name')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
