...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/cntry-table-nids-event-ml?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&geoip_country_name=${searchParams.get('geoip_country_name')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
