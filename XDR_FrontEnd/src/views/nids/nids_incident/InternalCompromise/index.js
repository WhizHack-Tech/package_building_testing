// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Incidents ( Internal Compromise Count )
//  ----------------------------------------------------------------------------------------------
...
const ActiveUsers = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-incident-int-compr?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-incident-internal-compromised-details?current_time=${rows.current_time}&past_time=${rows.past_time}&type_of_threat=${rows.type_of_threat}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
