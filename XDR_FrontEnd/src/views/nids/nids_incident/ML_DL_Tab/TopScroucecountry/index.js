...
const ChartjsBarChart = ({ tooltipShadow, gridLineColor, labelColor, successColorShade, lineChartPrimary, yellowColor }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-incident-ml-cntry-bar?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-incident-ml-dl-countries-details?current_time=${rows.current_time}&past_time=${rows.past_time}&geoip_country_name=${rows.geoip_country_name}&platform=${rows.platform}`} 
