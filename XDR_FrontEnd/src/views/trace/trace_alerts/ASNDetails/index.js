// ================================================================================================
//  File Name: ASNattackerdetails.js
//  Description: Details of the Trace ( Alerts ( ASN Details ) ).
//  ----------------------------------------------------------------------------------------------
...
const ANSDetailschart = ({ tooltipShadow, gridLineColor, labelColor, successColorShade, lineChartPrimary, yellowColor }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/trace-alert-attker-asn?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  filteredData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link
                            to={`/trace-alert-ans-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.geoip_asn_name}&name1=${rows.sensor_name}`}
