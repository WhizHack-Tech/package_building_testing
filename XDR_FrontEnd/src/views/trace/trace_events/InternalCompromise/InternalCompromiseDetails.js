...
//  Description: Details of the Trace ( Events ( Internal Compromise Details )).
//  ----------------------------------------------------------------------------------------------
...
const InternalcompromisedTable = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/int-attack-table-trace-event?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
