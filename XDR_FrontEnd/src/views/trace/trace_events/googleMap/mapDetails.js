...
const MapDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/geo-table-trace-event?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}&name1=${searchParams.get('name1')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
