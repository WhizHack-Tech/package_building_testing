// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Incidents ( Top Source Attacker cities Details)).
//  ----------------------------------------------------------------------------------------------
...
const ApexRadiarChart = () => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-attck-city-trace?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
        ...
        if (res.data.message_type === "success") {
          ...
          setCountriesTotal(res.data.pie_chart)
    ...
    <Card>
      <CardHeader className='d-flex justify-content-between align-items-sm-center align-items-start flex-sm-row flex-column'>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='xl'>
          <ModalHeader>{t('Top Source Attacker City ')}</ModalHeader>
          ...
          <ModalBody>
            <Table striped responsive>
              <thead>
                <tr>
                  <th>{t('Sensor Name')}</th>
                  <th>{t('Top Source Attacker City ')}</th>
                  <th>{t('Counts')}</th>
                  <th>{t('Actions')}</th>
                </tr>
              </thead>
                    ...
                    filteredData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/trace-incident-city-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.geoip_city}&name1=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                              {t('More Details')}
                            </Link>
                          </td>
                        </tr>
