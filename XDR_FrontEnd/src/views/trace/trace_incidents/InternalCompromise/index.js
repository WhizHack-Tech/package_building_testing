// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Incidents ( Internal Compromise Details )).
//  ----------------------------------------------------------------------------------------------
...
const internalcompromised = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/incident-internal-attack-trace?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader className='align-items-start pb-0'>
        <div>
          ...
          {/* <CardText>Internal Attack</CardText> */}
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/trace-incident-internal-compromised-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                          {t('More Details')}
                          </Link>
                        </td>
                      </tr>
      ...
      <CardBody style={{ height: '200px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          {checkApiData ? (
...
export default internalcompromised
