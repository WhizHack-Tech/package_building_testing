	...
	for {
		if h.isClient {
			...
			*/
			...
			/*
				Here we are collecting the artifacts we have received from the server.
				Often times this is the server our victim has contacted to download a malicious file.
				These artifacts get stored in a dedicated folder for further forensics analysis.
			*/
			logFolder := "http/"
