#  ===============================================================================================
#  File Name: hids_alert_wazuh_mitre_attack_page.py
...
def TopTactics(agent_id,indice_name,current_time,past_time, location_id, plan_id):
...
def RuleLevelByAttack(agent_id,indice_name,current_time,past_time, location_id, plan_id):
...
def RuleLevelByTactics(agent_id,indice_name,current_time,past_time, location_id, plan_id):
...
def AlertsEvolutionOverTime(agent_id,indice_name,current_time,past_time, condition, location_id, plan_id):
...
def MitreAttackByTactic(agent_id,indice_name,current_time,past_time, location_id, plan_id):
...
############################################# Adding view more functionality in all charts of Mitre Attack Page #############################
# -- 1.a. Mitre Alerts Evolution Over Time(line chart)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_alert_evolution_over_time(request):
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_alert_evolution_over_time_table(request):

    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_mitre_tactic"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_by_tactic(request):

    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="bar_chart"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_by_tactic_table(request):
