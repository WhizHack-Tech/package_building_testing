#  ===============================================================================================
#  File Name: hids_alert_wazuh_query.py
#  Description: This file contains code for each card on HIDS Alert Page.
#  Active URL: https://xdr-demo.zerohack.in/hids/alerts
...
from .hids_alert_mitre_attack_page import *

a = globals()
exclude_values = [0, None]


class HidsAlertSecurityEventPage(APIView):
    ...
    def get(self, request):
        ...
        if index_name_dict.get("hids_alert_agent") != None:
            ...
            # top_five_alert = TopFiveAlert(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            # top_five_rule_groups = TopFiveRuleGroups(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            # top_five_pci_dss_requirements = TopFivePciDssRequirements(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            alert_groups_evolution_security = AlertgroupsevolutionSecurity(
                agent_id, indice_name, current_time, past_time, location_id, plan_id
            )
            top_tactics = TopTactics(
                agent_id, indice_name, current_time, past_time, location_id, plan_id
            )
            # rule_level_by_attack = RuleLevelByAttack(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            # rule_level_by_tactics = RuleLevelByTactics(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            alerts_evolution_over_time = AlertsEvolutionOverTime(
                agent_id,
                indice_name,
                current_time,
                past_time,
            ...
            mitre_attack_by_tactic = MitreAttackByTactic(
                agent_id, indice_name, current_time, past_time, location_id, plan_id
            )
            ...
            return Response(
                {
                    ...
                    # "top_five_alert": top_five_alert,
                    # "top_five_rule_groups": top_five_rule_groups,
                    # "top_five_pci_dss_requirements": top_five_pci_dss_requirements,
                    "alert_groups_evolution_security": alert_groups_evolution_security,
                    "top_tactics": top_tactics,
                    # "rule_level_by_attack": rule_level_by_attack,
                    # "rule_level_by_tactics": rule_level_by_tactics,
                    "alerts_evolution_over_time": alerts_evolution_over_time,
...
def TopFiveAlert(agent_id, indice_name, current_time, past_time, location_id, plan_id):
...
def TopFivePciDssRequirements(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
    api_response = opensearch_conn_using_db(
        f"SELECT rule_pci_dss,COUNT(rule_pci_dss) count FROM {indice_name}-* WHERE rule_pci_dss IS NOT NULL AND  agent_id = {agent_id} AND attack_epoch_time >= {past_time} AND attack_epoch_time <= {current_time} GROUP BY rule_pci_dss ORDER BY count DESC LIMIT 5;",
        location_id,
        plan_id,
    )
    if api_response.get("total") not in exclude_values:
...
def AlertgroupsevolutionSecurity(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
...
############################################# Adding view more functionality in all charts of Security Event Page #############################
# 1.a Security Event Page hids alert queries(card name = Top 5 Alert) #TopFiveAlert #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_alert(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="pie_chart"
    )
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_alert_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_description"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_rule_groups(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_rule_groups_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_groups"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_rule_pci_dss(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_top_five_rule_pci_dss_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_pci_dss"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_page_rule_level_by_attack_id(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_page_rule_level_by_attack_id_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_mitre_id"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_page_rule_mitre_technique(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_mitre_attack_page_rule_mitre_technique_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_mitre_technique"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_alert_groups_evolution(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="line_chart"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_page_alert_groups_evolution_table(request):

    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table", rule="rule_hipaa"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_alert_security_logs_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_alert_agent", type="table"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
