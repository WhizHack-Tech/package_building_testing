#  ===============================================================================================
#  File Name: hids_incident_wazuh_query.py
#  Description: This file contains code for each card on HIDS Incident Page.
#  Active URL: https://xdr-demo.zerohack.in/hids/incidents
...
from .hids_incident_mitre_attack_page import *
...
def hids_incident_uniquedata_agentid(request):
    ...
    indice_name = index_name_dict.get(
        "hids_incident_agent"
    )  # opensearch_db indice name = xdr-hids-incident-jay-jasi9552-*
...
def hids_incident_security_alerts_func(
...
def hids_incident_top_five_alert_func(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
...
def hids_incident_top_five_rule_groups_func(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
    api_response = opensearch_conn_using_db(
...
def hids_incident_top_five_pci_dssrequirements_func(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
...
def hids_incident_alert_groups_evolution_security_func(
    agent_id, indice_name, current_time, past_time, location_id, plan_id
):
...
class HidsIncidentSecurityEventPage(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        if index_name_dict.get("hids_incident_agent") != None:
            indice_name = index_name_dict.get(
                "hids_incident_agent"
            )  # (indice name(column_name table) = hids_incident_agent)
            ...
            hids_incident_security_alerts = hids_incident_security_alerts_func(
                agent_id, indice_name, current_time, past_time, location_id, plan_id
            )
            # hids_incident_top_five_alerts = hids_incident_top_five_alert_func(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            # hids_incident_top_five_rule_groups = (
            #     hids_incident_top_five_rule_groups_func(
            #         agent_id, indice_name, current_time, past_time, location_id, plan_id
            #     )
            # )
            # hids_incident_top_five_pci_dss_requirements = (
            #     hids_incident_top_five_pci_dssrequirements_func(
            #         agent_id, indice_name, current_time, past_time, location_id, plan_id
            #     )
            # )
            hids_incident_alert_groups_evolution_security = (
                hids_incident_alert_groups_evolution_security_func(
                    agent_id, indice_name, current_time, past_time, location_id, plan_id
                )
            )
            hids_incident_top_tactics = hids_incident_top_tactics_func(
                agent_id, indice_name, current_time, past_time, location_id, plan_id
            )
            # hids_incident_rule_level_by_attack = hids_incident_rule_level_byattack_func(
            #     agent_id, indice_name, current_time, past_time, location_id, plan_id
            # )
            # hids_incident_rule_level_by_tactics = (
            #     hids_incident_rule_level_by_tactics_func(
            #         agent_id, indice_name, current_time, past_time, location_id, plan_id
            #     )
            # )
            hids_incident_alerts_evolution_over_time = (
                hids_incident_alerts_evolution_overtime_func(
                    agent_id,
                    indice_name,
                    current_time,
                    past_time,
            ...
            hids_incident_mitre_attack_by_tactic = (
                hids_incident_mitre_attackby_tactic_func(
                    agent_id, indice_name, current_time, past_time, location_id, plan_id
                )
            )
            ...
            return Response(
                {
                    "hids_incident_security_alerts": hids_incident_security_alerts,
                    # "hids_incident_top_five_alerts": hids_incident_top_five_alerts,
                    # "hids_incident_top_five_rule_groups": hids_incident_top_five_rule_groups,
                    # "hids_incident_top_five_pci_dss_requirements": hids_incident_top_five_pci_dss_requirements,
                    "hids_incident_alert_groups_evolution_security": hids_incident_alert_groups_evolution_security,
                    "hids_incident_top_tactics": hids_incident_top_tactics,
                    # "hids_incident_rule_level_by_attack": hids_incident_rule_level_by_attack,
                    # "hids_incident_rule_level_by_tactics": hids_incident_rule_level_by_tactics,
                    "hids_incident_alerts_evolution_over_time": hids_incident_alerts_evolution_over_time,
                    "hids_incident_mitre_attack_by_tactic": hids_incident_mitre_attack_by_tactic,
                }
            )
        else:
...
############################################# Adding view more functionality in all charts of Security Event Page in HIDS Incidents #############################
# 1.a Security Event Page hids incident queries(card name = Top 5 Alert) #TopFiveAlert #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_alert(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="pie_chart"
    )
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_alert_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table", rule="rule_description"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
# 2.a Security Event Page hids incident queries(card name = Top 5 Rule Groups) #TopFiveRuleGroups #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_rule_groups(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_rule_groups_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table", rule="rule_groups"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
# 3.a Security Event Page hids incident queries(card name = Top 5 Pci Dss Requirements) #TopFivePciDss #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_rule_pci_dss(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_top_five_rule_pci_dss_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table", rule="rule_pci_dss"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
# 4.a Mitre Attack Page hids incident queries(card name = Rule Level by attack ID) #RuleLevelAttackID #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_mitre_attack_page_rule_level_by_attack_id(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_mitre_attack_page_rule_level_by_attack_id_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table", rule="rule_mitre_id"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
# 5.a Mitre Attack Page hids incident queries(card name = Mitre Attack Techniques) #MitreAttackTechniques #pie chart
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_mitre_attack_page_rule_mitre_technique(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="pie_chart"
    )
    if parameters.pop("error"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_mitre_attack_page_rule_mitre_technique_table(request):
    parameters = pre_check_parameters(
        request,
        indice="hids_incident_agent",
        type="table",
        rule="rule_mitre_technique",
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_alert_groups_evolution(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="line_chart"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_page_alert_groups_evolution_table(request):

    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table", rule="rule_hipaa"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def hids_incident_security_logs_table(request):
    parameters = pre_check_parameters(
        request, indice="hids_incident_agent", type="table"
    )
    if parameters.pop("error"):
        return Response(parameters)
    else:
