// ==============================================================================================
//  File Name: attack-events.js
//  Description: Date fillter component
// ----------------------------------------------------------------------------------------------
...
const BreadCrumbs = props => {
  ...
  const loadding = useSelector((store) => store.attack_evnets_charts.loadding)
  const filter_val_show = useSelector((store) => store.attack_evnets_charts.filterValue)
  ...
  const filter_form = () => {
    ...
    if (defaultFilter === true) {
      ...
      if (checkPlateformVal === false) {
        ...
        dispatch(attack_events_charts({ platform: filter_val_show.defaultVals.platformSetVal, threat_severity: threatSeverity, start_date, end_date }))
        dispatch(attack_filter({ ...filter_value, platform: filter_val_show.platform }))
      }

      if (checkSeverityVal === false) {
        threatSeverityHandle(filter_val_show.defaultVals.severityVal)
        dispatch(attack_events_charts({ platform, threat_severity: filter_val_show.defaultVals.severitySetVal, start_date, end_date }))
        dispatch(attack_filter({ ...filter_value, threat_severity: filter_val_show.threat_severity }))
      }

      if (checkPlateformVal && checkSeverityVal) {
        dispatch(attack_events_charts({ platform, threat_severity: threatSeverity, start_date, end_date }))
        dispatch(attack_filter(filter_value))
      }

    } else {
      dispatch(attack_events_charts({ platform: filter_val_show.defaultVals.platformSetVal, threat_severity: filter_val_show.defaultVals.severitySetVal, start_date, end_date}))
      dispatch(attack_filter({...filter_val_show, start_date, end_date}))
    }
  ...
  if (loadding === true) {
    return <div style={{ backgroundColor: "rgb(0 0 0 / 37%)", position: "absolute", width: "100%", height: "100%", zIndex: 99999, paddingTop: "30rem" }} className="d-flex justify-content-center">
      <Spinner animation="border" variant="primary" type='grow' color='primary' />
    </div>
  }
