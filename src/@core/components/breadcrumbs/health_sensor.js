// ==============================================================================================
//  File Name: incidents_chart.js
//  Description: Details of the incidents chart component.
// ----------------------------------------------------------------------------------------------
...
import { filterValues } from '../../../views/healthCheck/store/health_sensor'
let setIntervalFun = null
const BreadCrumbs = props => {
  const { t } = useTranslation()
  const { breadCrumbTitle } = props
  const [isOpen, setIsOpen] = useState(false)
  const [filters, setFilters] = useState({ name: t('Last 7 Days'), value: 'last_7_days' })
  const [reloadVal, setReloadVal] = useState('off')
  const [timeVal, setTimeVal] = useState(1)
  const filterStore = useSelector(state => state.health_sensor)
        ...
        <div className="breadcrumb-left dropdown">
          <Form>
            <InputGroup className='mb-2'>
              ...
              {/* <Input value={filters.name || ''} placeholder= {t('Last 7 Days')} name="filter_name" onChange={(e) => setFilters({ ...filters, name: e.target.value })} /> */}
              <Input value={filterStore.filter_name || filters.name} placeholder= {t("Last 7 Days")} name="filter_name" onChange={(e) => setFilters({ ...filters, name: e.target.value })} />
            ...
            <Collapse isOpen={isOpen} style={{ position: 'fixed', zIndex: 1 }}>
              <Card>
                <CardBody>
                  ...
                  <Row>
                    <Col sm='6'>
                      <span className='join' onClick={() => funFilters({ name: t('Last 5 minutes'), value: "last_5_minutes" })} >{t('Last 5 minutes')}</span>
                      {/* <span className="collapse-close" onClick={() => setIsOpen(false)}>&times;</span> */}
                    </Col>
                    <Col>
                      <span className='join' onClick={() => funFilters({ name: t("Last 6 Hours"), value: "last_6_hours" })}>{t('Last 6 Hours')}</span>
                    </Col>
                  </Row>
                  <Row>
                    <Col>
                      <span className='join' onClick={() => funFilters({ name: t('Last 15 minutes'), value: "last_15_minutes" })}>{t('Last 15 minutes')}</span>
                    </Col>
                    <Col>
                      <span className='join' onClick={() => funFilters({ name: t("Last 12 Hours"), value: "last_12_hours" })}>{t('Last 12 Hours')}</span>
                    </Col>
                  </Row>
                  <Row>
                    <Col>
                      <span className='join' onClick={() => funFilters({ name: t("Last 30 minutes"), value: "last_30_minutes" })}>{t('Last 30 minutes')}</span>
                    </Col>
                    <Col>
                  ...
                  <Row>
                    <Col>
                      <span className='join' onClick={() => funFilters({ name: t("Last 1 Hour"), value: "last_1_hour" })}>{t('Last 1 hour')}</span>
                    </Col>
                    <Col>
