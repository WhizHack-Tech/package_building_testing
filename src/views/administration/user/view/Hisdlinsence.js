...
const Lisence = () => {
  ...
  const apiCall = () => {
    setApiLoading(true)
    axios.get(`/display-hids-license-management?location_id=${id}&activated_plan_id=${activated_plan_id}`, { headers: { Authorization: token() } }).then(res => {
  ...
  const lisenceUpdateForm = (event) => {
    ...
    axios({
      method: 'post',
      url: `/display-hids-license-management?activated_plan_id=${activated_plan_id}`,
      data: bodyFormData,
        ...
        <Row>
          ...
          <Col sm='3'>
            ...
            <Input type='select' name='xdr_hids_manager_status' >
              ...
              <option value="active" selected disabled>{apiData.xdr_hids_manager_status || ''}</option>
          ...
          <Col sm='3'>
            ...
            <Input type='select' name='current_sensor_status' defaultValue={apiData.control_server_domain || ''}>
              ...
              <option value={true} selected disabled>defaultValue={apiData.control_server_domain || ''}</option>
