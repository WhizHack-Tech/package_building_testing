...
const SwitchField = () => {
  ...
  const setEnvValue = (event) => {
    ...
    setLoading(true)
    axios
    .post('/update-pagepermission', envParams, { headers: { Authorization: token() } })
    .then((res) => {
      if (res.data.message_type === 'updated_successfully') {
        ...
        // Display an unsuccessful toast message here
        toast.unsuccessful('Failed to make changes')
      }
    })
    .catch((error) => {
      // Handle error
  ...
  useEffect(() => {
    axios.get(`/get-all-pagepermission?location_id=${id}&activated_plan_id=${activated_plan_id}`, { headers: { Authorization: token() } })
      .then(res => {
