...
const ApexRadiarChart = () => {
  ...
  if (chart_length > 0) {
    seriesData = chart_data.GeoipCityCount.series
    labelsData = chart_data.GeoipCityCount.labels
  }
    ...
    <Card>
      <CardHeader>
        ...
        >
          ...
          <PopoverBody>
            {t('Top Cities from where the attacks originate')}
          </PopoverBody>
        </Popover>
      </CardHeader>

      <CardBody>
        <Chart options={options} series={seriesData} type='pie' height={250} />
      </CardBody>
    </Card>
  )
}
