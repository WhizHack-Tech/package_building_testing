import React, { createRef, useState, useEffect } from "react"
import Tree from "react-d3-tree"
import { Card, CardBody } from "reactstrap"
import axios from "@axios"
import { token } from "@utils"
import { useTranslation } from 'react-i18next'
import { FullScreen, useFullScreenHandle } from "react-full-screen"
import { Maximize } from 'react-feather'
import { Link } from 'react-router-dom'
import "./graphStyle.css"
import "./globalCss.css"
const Graph = () => {
  const { t } = useTranslation()
  const handle = useFullScreenHandle()
  const [resData, setResData] = useState({})
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    axios
      .get("/onprimse-network-map", { headers: { Authorization: token() } })
      .then((res) => {
        if (res.data.message_type === "success") {
          setLoading(true)
          setResData(res.data.data)
        }
      })
  }, [])

  const containerStyles = {
    width: "100%",
    height: "100vh"
  }

  const renderNodeWithCustomEvents = ({ nodeDatum, toggleNode, wrapper }) => {
    // Horizontal orientation
    let y = 18
    // let y = -25;
    let x = -5

    return (
      <g ref={wrapper}>
        {/* <Tooltip
        title={<Typography>{nodeDatum.name}</Typography>}
        arrow
        placement="right"
      > */}
        <circle
          r="6"
          fill={nodeDatum.children ? "#ff9f43" : "#fb255e"}
          onClick={nodeDatum.children ? toggleNode : () => {
            console.log(nodeDatum)
          }
          }
        />
        {/* </Tooltip> */}
        <text
          fill="#968df4"
          strokeWidth="0.1"
          fontSize="10px"
          x={x.toString()}
          y={y.toString()}
        >
          {nodeDatum.name}
        </text>
      </g>
    )
  }

  // const [translate, containerRef] = useCenteredTree();
  const wrapper = createRef()
  return (
    <Card>
      <div className="content-header-right text-md-right ">
          <Link color="primary" onClick={handle.enter}>
            <Maximize size={30} />
          </Link>
          <h6>{t("Fullscreen")}</h6>
      </div>
      <FullScreen handle={handle}>
        <CardBody>
          <div style={containerStyles}>
            {loading ? (
              <Tree
                data={resData}
                collapsible={true}
                enableLegacyTransitions={true}
                initialDepth={"5"}
                orientation="horizontal"
                depthFactor="200"
                translate={{ x: 200, y: 300 }}
                shouldCollapseNeighborNodes={false}
                nodeSize={{ y: 30 }}
                // translate={translate}
                renderCustomNodeElement={(props) => renderNodeWithCustomEvents({ ...props, wrapper })
                }
              />
            ) : (
              <p className='data-not-fount'>
                {t("Data not available at this moment")}
              </p>
            )}
          </div>
        </CardBody>
      </FullScreen>
    </Card>
  )
}
export default Graph
