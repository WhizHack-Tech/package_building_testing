...
const ApexRadiarChart = () => {
  ...
  const apiLogic = () => {
    setApiLoader(true)
    axios.get(`/hids-alert-mitre-attack-event-page-rule-mitre-technique?agent_id=${searchParams.get('agent_id')}&condition=${searchParams.get('condition')}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/hids_alert_rule_mitre_technique?current_time=${rows.current_time}&past_time=${rows.past_time}&agent_id=${rows.agent_id}&rule_mitre_technique=${rows.rule_mitre_technique}&agent_ip=${rows.agent_ip}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
