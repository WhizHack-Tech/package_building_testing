...
const DataTablesBasic = () => {
  ...
  const DetailsApiLogic = () => {
    setApiLoader(true)
    axios.get(`/hids-event-security-log-table?agent_id=${searchParams.get('agent_id')}&condition=${searchParams.get('condition')}&past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}`, { headers: { Authorization: token() } })
        .then(res => {
            setApiLoader(false)
