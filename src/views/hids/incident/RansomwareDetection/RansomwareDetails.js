...
const TopAttackedCount = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/incident-hids-ransomware-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&agent_id=${searchParams.get('agent_id')}&agent_ip=${searchParams.get('agent_ip')}&ransomware_count=${searchParams.get('ransomware_count')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
