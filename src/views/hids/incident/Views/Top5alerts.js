...
const ApexRadiarChart = () => {
  ...
  const apiLogic = () => {
    setApiLoader(true)
    axios.get(`/hids-incident-security-event-page-top-alert?agent_id=${searchParams.get('agent_id')}&condition=${searchParams.get('condition')}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='xl'>
          <ModalHeader>{t('Top 5 Alerts')}</ModalHeader>
          <ModalBody>
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <span className='align-middle fw-bold'>{rows.rule_description}</span>
                        </td>
                        <td>
                          <span className='align-middle fw-bold'>{rows.agent_count}</span>
                        </td>
                        <td>
                          <Link to={`/hids-incident-security_events_top_5_alerts?current_time=${rows.current_time}&past_time=${rows.past_time}&agent_id=${rows.agent_id}&rule_description=${rows.rule_description}&agent_ip=${rows.agent_ip}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
