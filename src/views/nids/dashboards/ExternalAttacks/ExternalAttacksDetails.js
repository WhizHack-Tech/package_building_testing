...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`http://127.0.0.1:8000/critical-threats-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`)
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
            <CardHeader>
                <CardTitle>
                External Attacks : {searchParams.get('name')}
                </CardTitle>
            </CardHeader>
