...
const PieCharts = () => {
  ...
  const ChartApiLogic = () => {
    ...
    axios.get(`/nids-attck-event-port?condition=${filterState.values ? filterState.values : 'today'}`)
      .then(res => {
        setApiLoader(false)
        if (res.data.message_type === "success") {
          ...
          if (res.data.ports.series.length > 0) {
            const tableDataRows = res.data.ports.series.map((series, index) => ({
              series,
              labels: res.data.ports.labels[index],
  ...
  return (
    <Card>
      ...
      <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
        <ModalHeader>Most Attacked Ports</ModalHeader>
        <ModalBody>
                ...
                shortTableData.map((rows, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        <Link to={`/most-attacked-ports?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.labels}`}>
