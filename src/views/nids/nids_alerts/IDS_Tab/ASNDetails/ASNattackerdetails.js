...
const ANSDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-alert-asn-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&geoip_asn_name=${searchParams.get('geoip_asn_name')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
