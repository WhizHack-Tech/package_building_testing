// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Alerts ( Internal Actack Count )
//  ----------------------------------------------------------------------------------------------
...
const internalcompromised = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/internal-attack-card-nids-alert?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                    ...
                    return (
                      <tr key={index}>
                       <td>
                        ...
                        <td>
                          <Link to={`/nids-alert-internal-compromised-details?current_time=${rows.current_time}&past_time=${rows.past_time}&type_of_threat=${rows.type_of_threat}&platform=${rows.platform}`} className='btn-sm btn-outline-primary m-2'>
                          {t('More Details')}
                          </Link>
                        </td>
                      </tr>
