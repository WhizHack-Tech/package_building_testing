...
//  Description: Details of the NIDS Alerts ( Lateral Movement Actack Count )
//  ----------------------------------------------------------------------------------------------
...
const LateralMovementTable = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-alert-lateral-mov-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&type_of_threat=${searchParams.get('type_of_threat')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
             <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                <CardTitle tag='h4'> {t("Lateral Movement")} : {searchParams.get("type_of_threat")}</CardTitle>
...
export default LateralMovementTable