// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Alerts ( External Details ).
//  ----------------------------------------------------------------------------------------------
...
const ExternalAttackchart = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-alert-ext?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader className='align-items-start pb-0'>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
          {/* <ModalHeader>External Attack</ModalHeader> */}
                    ...
                    shortTableData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/nids-alert-external-attack-count?current_time=${rows.current_time}&past_time=${rows.past_time}&platform=${rows.platform}&type_of_threat=${rows.type_of_threat}`} className='btn-sm btn-outline-primary m-2'>
                              More Details
                            </Link>
                          </td>
                        </tr>
