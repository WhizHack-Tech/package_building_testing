// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Events ( Attack Counts )
//  ----------------------------------------------------------------------------------------------
...
const ApexLineChart = ({ direction, warning }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-event-attack-freq?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
    ...
    <Card>
      <CardHeader className='align-items-start pb-0'>
        ...
        <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
          <ModalHeader>Frequency of Attack</ModalHeader>
          <ModalBody>
            <Table striped responsive>
              <thead>
                <tr>
                  <th>{t('Frequency of Attack')}</th>
                  <th>{t('Counts')}</th>
                  <th>{t('Actions')}</th>
                </tr>
              </thead>
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-event-frequency-details/?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.name}`} className='btn-sm btn-outline-primary m-2'>
                          {t('More Details')}
                          </Link>
                        </td>
                      </tr>
