// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Events ( Critical Threats)
//  ----------------------------------------------------------------------------------------------
...
const ActiveUsers = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/nids-dashboard-critical-threats?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                  ...
                  (shortTableData.length === 0 || checkApiData === false) ? (
                    ...
                    shortTableData.map((rows, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/nids-alert-critical-threats-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.threat_name}`} className='btn-sm btn-outline-primary m-2'>
                            {t('More Details')}
                          </Link>
                        </td>
                      </tr>
