// ================================================================================================
//  File Name: index.js
//  Description: Details of the NIDS Events IDS ( Target Host Details )
//  ----------------------------------------------------------------------------------------------
...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-event-freq-targetted-host?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
        .then(res => {
            setApiLoader(false)
