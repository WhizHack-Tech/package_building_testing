...
//  Description: Details of the NIDS Incidents IDS ( target Port )
//  ----------------------------------------------------------------------------------------------
...
const MapDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-incident-target-port-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&target_port=${searchParams.get('target_port')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
