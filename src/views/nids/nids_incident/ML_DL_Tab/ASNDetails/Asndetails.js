...
const TopAttackedDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-incident-ml-asn-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&geoip_asn_name=${searchParams.get('geoip_asn_name')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
             <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                <CardTitle tag='h4'> {t("ASN Details")} : {searchParams.get("geoip_asn_name")}</CardTitle>
