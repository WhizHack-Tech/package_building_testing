...
const DetectedThreatType = () => {
    ...
    const apiLogic = () => {
        ...
        axios.get(`/nids-incident-ml-detected-threat?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
                        ...
                        tableData.map((rows, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>
                                        <Link to={`/nids-incident-ml-dl-details?current_time=${rows.current_time}&past_time=${rows.past_time}&type=${type}&name=${rows.name}`} className='btn-sm btn-outline-primary m-2'>
                                            {t('More Details')}
                                        </Link>
                                    </td>
                                </tr>
