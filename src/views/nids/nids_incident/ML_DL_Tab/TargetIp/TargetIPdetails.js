...
const MapDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-incident-ml-target-ip-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&target_ip=${searchParams.get('target_ip')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
