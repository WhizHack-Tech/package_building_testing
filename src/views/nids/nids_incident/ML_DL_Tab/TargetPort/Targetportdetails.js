...
//  Description: Details of the NIDS Incidents ML & DL ( Target Port )
//  ----------------------------------------------------------------------------------------------
...
const MapDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/nids-incident-ml-trgt-port-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&target_port=${searchParams.get('target_port')}&platform=${searchParams.get('platform')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
