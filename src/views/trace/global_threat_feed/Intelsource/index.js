// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Golbal Threat Feed ( Intel Source Details ) ).
//  ----------------------------------------------------------------------------------------------
...
const CityChart = () => {
  ...
  const ChartApiLogic = () => {
    ...
    axios.get(`/glog-trace-intel-source?condition=${filterState.values ? filterState.values : 'last_24_hours'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
  ...
  return (
    <Card>
      ...
      <Modal isOpen={modalBox} toggle={() => setModalBox(!modalBox)} size='lg'>
        <ModalHeader>{t('Intel Source')}</ModalHeader>
        <ModalBody>
          <Table striped responsive>
            <thead>
              <tr>
                <th>{t('Intel Source')}</th>
                <th>{t('Counts')}</th>
                <th>{t('Actions')}</th>
              </tr>
            </thead>
                  ...
                  shortTableData.map((rows, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/trace-gobal-intel-source-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.labels}`} className='btn-sm btn-outline-primary m-2'>
                          {t('More Details')}
                          </Link>
                        </td>
                      </tr>
