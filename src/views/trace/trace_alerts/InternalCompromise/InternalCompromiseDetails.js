...
//  Description: Details of the Trace ( Alerts ( Internal Compromised Details )).
//  ----------------------------------------------------------------------------------------------
...
const InternalcompromisedTable = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/trace-alert-int-attack-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
