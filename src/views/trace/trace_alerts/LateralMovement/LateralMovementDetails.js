...
const LateralMovementTable = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/trace-alert-lateral-mov-table?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
