// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Alerts ( Lateral Movement Details )).
//  ----------------------------------------------------------------------------------------------
...
const LateralMovementchart = ({ success }) => {
  ...
  const apiLogic = () => {
    ...
    axios.get(`/trace-alert-lateral-mov?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then(res => {
        setApiLoader(false)
                    ...
                    filteredData.map((rows, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/trace-alert-lateral-movement-details?current_time=${rows.current_time}&past_time=${rows.past_time}&name=${rows.sensor_name}`} className='btn-sm btn-outline-primary m-2'>
                              {t('More Details')}
                            </Link>
                          </td>
                        </tr>
