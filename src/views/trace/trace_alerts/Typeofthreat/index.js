// ================================================================================================
//  File Name: index.js
//  Description: Details of the Trace ( Alerts ( Tyoe of Threat Details )).
//  ----------------------------------------------------------------------------------------------
...
const ExternalVSInternal = ({ success }) => {
  ...
  const apiLogic = () => {
    setApiLoader(true)
    axios
      .get(`/trace-alert-threat-type?condition=${filterState.values ? filterState.values : 'last_1_hour'}`, { headers: { Authorization: token() } })
      .then((res) => {
        setApiLoader(false)
    ...
    <Card className='earnings-card'>
      <CardBody>
        ...
        {checkApiData ? (
          <Row>
            <Col xs='6'>
              <CardTitle className='mb-1' tag='h4'>{t('Type of Threat')}</CardTitle>
