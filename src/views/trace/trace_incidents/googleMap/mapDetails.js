...
const MapDetails = () => {
    ...
    const DetailsApiLogic = () => {
        setApiLoader(true)
        axios.get(`/incident-geo-table-trace?past_time=${searchParams.get('past_time')}&current_time=${searchParams.get('current_time')}&name=${searchParams.get('name')}&name1=${searchParams.get('name1')}`, { headers: { Authorization: token() } })
            .then(res => {
                setApiLoader(false)
    ...
    return (
        <Card className='overflow-hidden'>
             <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                <CardTitle tag='h4'> {t("Attacker Geo-Locations")} : {searchParams.get("name")}</CardTitle>
