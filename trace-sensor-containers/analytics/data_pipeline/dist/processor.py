...
def honeynet_data_load(honeynet_path):
    ...
    honeynet_dataframe = honeynet_dataframe.dropna(axis=0, subset=['src_ip']).reset_index(drop = True)
    honeynet_dataframe['dest_ip'] = honeynet_dataframe['dest_ip'].fillna('0.0.0.0')
...
def dpi_data_load(dpi_path):
    ...
    dpi_dataframe['attack_epoch_time'] = pd.to_datetime(dpi_dataframe['timestamp'])
    dpi_dataframe = dpi_dataframe.drop(['timestamp'], axis=1)
...
def data_cleaner(honeynet_dataframe):
    ...
    honeynet_dataframe['dst_host_type']= honeynet_dataframe['dest_ip'].apply([lambda x: 'internal' if ip_address(x).is_private else 'external'])
